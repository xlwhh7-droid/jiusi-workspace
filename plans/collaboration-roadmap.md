...
### 🟣 阶段四：仿生进化 (Bio-Mimetic Evolution)
*目标：基于[生物与AI记忆机制对比研究](../reading/articles/bio-ai-memory.md)，引入类脑机制。*
- [ ] **4.1 记忆分层**：
    - **海马体 (Fast)**：`memory/` (每日日志)，记录情景。
    - **新皮层 (Slow)**：`MEMORY.md` + 向量库，提取规律。
- [ ] **4.2 睡眠计算 (Sleep-time Compute)**：利用空闲周期 (02:00-08:00) 自动执行记忆整理、去重和压缩，而非单纯待机。
- [ ] **4.3 主动遗忘**：引入 TTL 和重要性权重，定期修剪无用记忆，对抗熵增。
